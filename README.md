[2 index.html](https://github.com/user-attachments/files/31302657/2.index.html)
<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
  body {
    min-height: 100vh;
    background: linear-gradient(135deg, #ffafbd 0%, #ffc3a0 50%, #ff9a9e 100%);
    display: flex;
  ...
  h1 { font-size: 2em; color: #d63384; margin-bottom: 10px; }
  .name {
  ...
  .day-btn:hover { transform: translateY(-3px); border-color: #ff6b9d; color: #d63384; }
  .day-btn.active {
    background: linear-gradient(135deg, #ff6b9d, #d63384);
    color: white;
  ...
  .day-title { font-size: 1.2em; color: #d63384; margin-bottom: 6px; font-weight: bold; }
  .day-desc { color: #555; line-height: 1.5; font-size: 0.95em; }
  
  .activity-section, .location-section, .walk-section, .fun-section, .dishes-section, .comment-section {
    animation: slideIn 0.5s ease-out;
    margin-bottom: 20px;
  }
  .activity-options, .location-options, .walk-options, .fun-options, .dishes-options {
    display: grid;
    gap: 10px;
  }
  .activity-options { grid-template-columns: repeat(3, 1fr); }
  .location-options, .walk-options { grid-template-columns: repeat(2, 1fr); }
  .fun-options { grid-template-columns: repeat(3, 1fr); }
  .dishes-options { grid-template-columns: repeat(3, 1fr); }
  
  .activity-card, .location-card, .walk-card, .fun-card, .dish-card {
    background: white;
    border: 2px solid #ffd1dc;
    border-radius: 16px;
    padding: 15px 10px;
  ...
  .activity-card:hover, .location-card:hover, .walk-card:hover, .fun-card:hover, .dish-card:hover {
    transform: translateY(-5px);
    border-color: #ff6b9d;
    box-shadow: 0 8px 20px rgba(255, 107, 157, 0.2);
  }
  .activity-card.active, .location-card.active, .walk-card.active, .fun-card.active, .dish-card.active {
    background: linear-gradient(135deg, #ff6b9d, #d63384);
    border-color: transparent;
    color: white;
    box-shadow: 0 8px 20px rgba(214, 51, 132, 0.4);
  }
  .activity-card .a-icon, .location-card .l-icon, .walk-card .w-icon, .fun-card .f-icon, .dish-card .d-icon {
    font-size: 28px;
    margin-bottom: 6px;
  }
  .activity-card .a-name, .location-card .l-name, .walk-card .w-name, .fun-card .f-name, .dish-card .d-name {
    font-size: 0.9em;
    font-weight: 600;
    color: #555;
  }
  .activity-card.active .a-name, .location-card.active .l-name, .walk-card.active .w-name, .fun-card.active .f-name, .dish-card.active .d-name { color: white; }
  
  .activity-details, .location-details, .walk-details, .fun-details, .dishes-summary {
    background: linear-gradient(135deg, #fff0f5, #ffe4ec);
    padding: 18px;
    border-radius: 15px;
    margin-top: 15px;
  ...
  .activity-details p, .location-details p, .walk-details p, .fun-details p, .dishes-summary p {
    color: #555;
    line-height: 1.5;
    font-size: 0.95em;
    margin-bottom: 8px;
  ...
  .comment-input::placeholder { color: #bbb; font-style: italic; }
  .char-counter {
    text-align: right;
    font-size: 0.8em;
    color: #aaa;
  ...
  .yes:hover { transform: scale(1.08); box-shadow: 0 8px 20px rgba(214, 51, 132, 0.6); }
  .yes:disabled {
    opacity: 0.7;
    cursor: not-allowed;
    transform: none;
  ...
  .chosen-info p { margin: 6px 0; color: #555; }
  .chosen-info strong { color: #d63384; }
  ...
  @media (max-width: 500px) {
    ...
    .activity-options, .location-options, .walk-options, .fun-options, .dishes-options { grid-template-columns: repeat(2, 1fr); }
    h1 { font-size: 1.6em; }
...
<div class="card" id="mainCard">
  ...
  <p class="subtitle">Давай спланируем идеальное свидание ✨</p>
  ...
  <div class="comment-section">
    ...
    <div class="comment-hint">Можешь написать что угодно: аллергии, предпочтения, любимые фильмы, или просто тёплые слова — я всё учту 💕</div>
    <textarea id="commentInput" class="comment-input" placeholder="Например: не ем лук, хочу безалкогольное вино, или просто «люблю тебя» 💕" maxlength="300" oninput="updateCharCounter()"></textarea>
...
<div class="card success" id="successCard">
  ...
  <p>Обещаю, это будет волшебный вечер ✨</p>
</div>

<script>
  // ⚙️ ВСТАВЬ СЮДА URL СВОЕГО GOOGLE APPS SCRIPT (инструкция ниже)
  const GOOGLE_SCRIPT_URL = 'ВСТАВЬ_СЮДА_URL_ТВОЕГО_СКРИПТА';
  ...
  function resetAll() {
    currentActivity = null; currentLocation = null; currentWalk = null; currentFun = null; currentDishes = [];
    document.querySelectorAll('.activity-card, .location-card, .walk-card, .fun-card, .dish-card').forEach(c => c.classList.remove('active'));
  ...
  function renderActivity(actKey) {
    ...
    currentLocation = null; currentWalk = null; currentFun = null; currentDishes = [];
    document.querySelectorAll('.location-card, .walk-card, .fun-card, .dish-card').forEach(c => c.classList.remove('active'));
    document.getElementById('locationDetails').innerHTML = '';
    document.getElementById('walkDetails').innerHTML = '';
    document.getElementById('funDetails').innerHTML = '';
    renderDishesList();
  ...
  async function sendToGoogleSheets(data) {
    if (!GOOGLE_SCRIPT_URL || GOOGLE_SCRIPT_URL === 'ВСТАВЬ_СЮДА_URL_ТВОЕГО_СКРИПТА') {
      console.warn('Google Script URL не настроен');
      return { success: false, error: 'URL не настроен' };
    }

    try {
      // Используем no-cors, чтобы обойти проблемы с CORS
      // Apps Script всё равно получит данные и запишет их
  ...
  async function sayYes() {
    ...
    // Показываем экран успеха (в любом случае — не ломаем романтику!)
    ...
    if (sheetResult.success) {
      statusDiv.innerHTML = '<div class="sheet-status success">✅ Твой выбор сохранён 💕</div>';
    } else {
      statusDiv.innerHTML = '<div class="sheet-status error">⚠️ Не удалось сохранить, но я всё запомню 💕</div>';
    }
